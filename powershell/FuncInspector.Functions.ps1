...
function ConvertTo-FiCondText([string]$c) {
    # 1つの条件式を読みやすい日本語に。該当しなければ原文のまま。
    $c = $c.Trim()
    if ($c -match '^defined\s*\(\s*(\w+)\s*\)$') { return "$($Matches[1]) を定義" }
    if ($c -match '^!\s*defined\s*\(\s*(\w+)\s*\)$') { return "$($Matches[1]) は未定義" }
    if ($c -match '^!\(\s*defined\s*\(\s*(\w+)\s*\)\s*\)$') { return "$($Matches[1]) は未定義" }
    if ($c -match '^(\w+)\s*==\s*(.+)$') { return "$($Matches[1]) = $($Matches[2])" }
    if ($c -match '^(\w+)\s*!=\s*(.+)$') { return "$($Matches[1]) ≠ $($Matches[2])" }
    if ($c -match '^(\w+)\s*>=\s*(.+)$') { return "$($Matches[1]) が $($Matches[2]) 以上" }
    if ($c -match '^(\w+)\s*<=\s*(.+)$') { return "$($Matches[1]) が $($Matches[2]) 以下" }
    if ($c -match '^(\w+)\s*>\s*(.+)$') { return "$($Matches[1]) が $($Matches[2]) 超" }
    if ($c -match '^(\w+)\s*<\s*(.+)$') { return "$($Matches[1]) が $($Matches[2]) 未満" }
    if ($c -match '^!\(\s*(.+?)\s*\)$') { return "「$($Matches[1])」でない" }
    if ($c -match '^(\w+)$') { return "$($Matches[1]) を定義(非0)" }
    return $c
}
function Get-FiReachCondition([string]$File, [int]$Line) {
    # File の Line(初出=その #if 行)に到達するための「囲み条件」を読みやすく返す。
    # 評価はせず、入れ子の各 #if/#ifdef/#elif/#else の枝条件を「かつ」で連結する。
    ...
    if ($stack.Count -eq 0) { return '無条件（常に対象）' }
    return (($stack | ForEach-Object { ConvertTo-FiCondText $_.Cur }) -join ' かつ ')
}
    ...
    $swlv.Add_CellClick({
            ...
            if ($e.RowIndex -ge 0) {
                ...
                if ($row.Tag) {
                    ...
                    $status.Text = ("この関数群が出る条件 … {0}（{1}）" -f $cond, $row.Cells['Sw'].Value)
                }
            }
        })
