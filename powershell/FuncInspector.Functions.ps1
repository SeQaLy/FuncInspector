...
function Initialize-FiNative {
    ...
    try { $script:FiNativeType = [FuncInspectorNativeV9.Engine]; return $true } catch {}
    $code = @'
using System;
using System.Collections.Generic;
namespace FuncInspectorNativeV9 {
...
'@
    try {
        ...
        $script:FiNativeType = [FuncInspectorNativeV9.Engine]
        return $true
    }
    ...
    $timer.Add_Tick({
            ...
            if ($s.Done) {
                ...
                elseif ($s.Mode -eq 'switch') {
                    ...
                    $status.Text = ("完了: {0} 個のスイッチ (行クリックで到達条件を表示)" -f $agg.Count)
                }
                else {
...
■ 検出対象の決まり方（重要）
  ...
  (4)「include解決(重い)」にチェック ＝ 完全 cpp 準拠
      → #include "..." をたどり、別ファイル(config.h 等)の #define を反映。
        対象フォルダは自動検索（GUIに -I 欄は無し。ツリー外ヘッダは上位フォルダをスキャン）。
        ・値あり #define X 10 ＝その値 / フラグ(値なし #define X)＝定義(ON)。
          どちらもファイルをまたいで反映。条件付き定義の連鎖も自動
          （CFG_UFS_ENABLE が定義/選択されると中の #define CFG_ENABLE 1 が効く）。
        ・実値と違う枝（CFG_NUM=10 のとき #if CFG_NUM==5）は出ない。
        ・探索: ソース定義のスイッチを外すなら -U、値を変えるなら -D（常に優先）。
          「全部OFFから選択で足す」なら通常(軽量)=選択スイッチのみ有効を使う。
