<#
.SYNOPSIS
    FuncInspector (関数定義版) - プロファイル埋め込み用。

.DESCRIPTION
    このファイルは「関数を定義するだけ」で、読み込んだ時点では何も実行しません。
    PowerShell の起動プロファイル ($PROFILE) にドットソースで読み込んでおくと、
    使いたいときに Invoke-FuncInspector コマンドで呼び出せます。

    出力フォーマット:  file.c,line,funcname

    機能は powershell/FuncInspector.ps1 と同じ:
...
.NOTES
    ◆ プロファイルへの埋め込み方
      1) $PROFILE のパスを確認:   $PROFILE
      2) プロファイルに次の1行を追記 (パスは環境に合わせる):
             . "Z:\develop\FuncInspector\powershell\FuncInspector.Functions.ps1"
      3) PowerShell を再起動 (または  . $PROFILE  で再読込)
...
function Find-CFunctions {
    <#
    .SYNOPSIS  1ファイルを解析し File/Line/Function オブジェクトを返す。
    #>
    [CmdletBinding()]
...
function Show-FuncInspectorGui {
    ...
    $tb.Location = '120,12'; $tb.Size = '480,24'; $tb.Anchor = 'Top,Left,Right'
    $form.Controls.Add($tb)

    $btnFolder = New-Object System.Windows.Forms.Button
    $btnFolder.Text = 'フォルダ...'; $btnFolder.Location = '610,11'; $btnFolder.Size = '90,25'; $btnFolder.Anchor = 'Top,Right'
    $btnFolder.Add_Click({
    ...
    $btnFile.Text = 'ファイル...'; $btnFile.Location = '705,11'; $btnFile.Size = '90,25'; $btnFile.Anchor = 'Top,Right'
    $btnFile.Add_Click({
    ...
    $lv.Location = '10,80'; $lv.Size = '785,390'; $lv.Anchor = 'Top,Bottom,Left,Right'
    ...
    $status.Location = '10,475'; $status.Size = '500,20'; $status.Text = '準備完了'; $status.Anchor = 'Bottom,Left'
    $form.Controls.Add($status)

    $script:FIguiRows = @()

    $btnScan = New-Object System.Windows.Forms.Button
    $btnScan.Text = 'スキャン'; $btnScan.Location = '600,495'; $btnScan.Size = '90,28'; $btnScan.Anchor = 'Bottom,Right'
    $btnScan.Add_Click({
            if (-not $tb.Text.Trim()) {
    ...
    $btnSave.Text = 'CSV 保存'; $btnSave.Location = '700,495'; $btnSave.Size = '95,28'; $btnSave.Anchor = 'Bottom,Right'
    $btnSave.Add_Click({
            if (-not $script:FIguiRows -or $script:FIguiRows.Count -eq 0) {
...
function Invoke-FuncInspector {
    <#
    .SYNOPSIS
        C ソースから関数名を抽出する (出力: file,line,funcname)。
    .EXAMPLE
        Invoke-FuncInspector -Path .\src
    .EXAMPLE
        Invoke-FuncInspector -Path .\src -Out result.csv -Header
    ...
    param(
        ...
        [switch]$Header,
        [switch]$Gui,
        [switch]$AsObject   # 指定時はテキストでなくオブジェクトを返す
    )

    if ($Gui -or -not $Path -or $Path.Count -eq 0) {
        Show-FuncInspectorGui
        return
    }
    ...
    if ($Out) {
        ...
        Write-Host ("{0} 件検出" -f $rows.Count) -ForegroundColor DarkGray
    }
}
