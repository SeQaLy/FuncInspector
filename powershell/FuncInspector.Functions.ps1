...
function Initialize-FiNative {
    ...
    try { $script:FiNativeType = [FuncInspectorNativeV8.Engine]; return $true } catch {}
    $code = @'
using System;
using System.Collections.Generic;
namespace FuncInspectorNativeV8 {
    ...
    static void PpProcess(char[] b, Dictionary<string,string> d, HashSet<string> pinned, string baseDir, bool collect, int depth, string[] incDirs, HashSet<string> seen){
      ...
      while(ls<=n){ int le=ls; while(le<n && b[le]!='\n') le++;
        ...
        if(kind!=0){ blank=true;
          ...
          else if(kind==7){ if(emit){ int after; string nm=FirstIdentAfter(b,rest,le,out after); if(nm!=null && !pinned.Contains(nm)){ int vs=after; while(vs<le&&(b[vs]==' '||b[vs]=='\t'))vs++; int ve=le; while(ve>vs&&(b[ve-1]==' '||b[ve-1]=='\t'||b[ve-1]=='\r'))ve--; if(ve>vs){ string val=new string(b,vs,ve-vs); d[nm]=val; } /* 値あり=反映/値なし(フラグ)=無視 */ } } }
...
'@
    try {
        ...
        $script:FiNativeType = [FuncInspectorNativeV8.Engine]
        return $true
    }
    ...
    foreach ($line in ($Clean -split "`n")) {
        ...
        if ($m.Success) {
            ...
            switch ($kind) {
                ...
                'endif' { if ($stack.Count) { $stack.RemoveAt($stack.Count - 1) } }
                'define' { if (Test-FiEmitting $stack) { $idm = $script:FiRxId.Match($rest); if ($idm.Success -and -not $Pinned.ContainsKey($idm.Value)) { $after = $rest.Substring($idm.Index + $idm.Length).Trim(); if ($after -ne '') { $Defines[$idm.Value] = $after } } } }
...
■ 検出対象の決まり方（重要）
  ...
  (4)「include解決(重い)」にチェック
      → #include "..." をたどり、別ファイル(config.h 等)の #define を反映。
        対象フォルダは自動検索。-I 欄は通常不要（上書き/追加用）。
        ・フラグ（値なし #define X）＝スイッチ扱い＝選択した時だけ ON。
          例: #define CFG_AAA + #ifdef CFG_AAA は未選択なら出ない。
        ・値あり #define X 10 ＝そのまま反映（ファイルをまたいでも）。
          条件付き定義の連鎖も自動（CFG_UFS_ENABLE を選ぶと
          中の #define CFG_ENABLE 1 が効き、その関数まで出る）。
        ・実値と違う枝（CFG_NUM=10 のとき #if CFG_NUM==5）は出ない。
          見たい時はその値を選ぶ（-D）か「全コード有効」。
