...
スイッチ追跡モードの `#define` 反映ルール（`-D`/`-U`・グリッド選択は常に最優先）:
- **フラグ（値なし `#define X`）＝ スイッチ＝選択駆動**。ソースの定義は無視し、
  **選択した時だけ ON**（例 `#define CFG_AAA` ＋ `#ifdef CFG_AAA` は未選択なら出ない）。
- **値あり `#define X 10`＝ そのまま反映**（ファイルをまたいでも）。条件付き定義の
  連鎖も自動（例: `CFG_UFS_ENABLE` を選ぶと中の `#define CFG_ENABLE 1` が反映され、
  `#if CFG_ENABLE==1` の関数まで出る）。
- 値が実値と違う枝（例 `CFG_NUM=10` のとき `#if CFG_NUM==5`）は出ません（実ビルド準拠）。
  見たい時はその値を選ぶ（`-D CFG_NUM=5`）か「全コード有効」。
